    ...
    static void CompareNeighboringElements(int[] array, int index)
    {
        ...
        for (int i = index - 1; i <= index + 1; i += 2)
        {
            if (IsInRange(array, i))
            {
                ...
                {
                    Console.WriteLine("Element {0} is bigger than selected element", i);
                }
                else if (array[i] < array[index])
                {
                    Console.WriteLine("Element {0} is smaller than selected element", i);
                }
                else if (array[i] == array[index])        
	            {
                    Console.WriteLine("Element {0} is equal to selected element", i);
	            }
            }
        }        
    }
